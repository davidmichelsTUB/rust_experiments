        ...
        for cols in params["cols"]:
            for kwargs in params["kwargs"]:
                for rows in params["rows"]:
                    ...
                    sklearn_mean, sklearn_std, _ = time_it(lambda: sklearn_transformer.fit(X))

                    # Fit and transform using parallel implementation
                    parallel_transformer = ParallelQuantileTransformer(**kwargs, n_jobs=njobs)
                    parallel_mean, parallel_std, _ = time_it(lambda: parallel_transformer.fit(X))
                    # Compare results
                    ...
                    if not Path(__file__).parent.joinpath("results_fit.csv").exists():
                        results_csv = pd.DataFrame([results])
                        results_csv.to_csv(Path(__file__).parent / "results_fit.csv", index=False)
                    else:
                        results_csv = pd.read_csv(Path(__file__).parent / "results_fit.csv")
