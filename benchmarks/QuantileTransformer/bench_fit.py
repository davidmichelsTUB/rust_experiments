...
params = {
    ...
    "cols" : [500, 1000, 5000],
    "kwargs" : [{"n_quantiles" : a, "output_distribution" : b, "random_state" : 42} for a in [100, 1000, 10000] for b in ['normal', 'uniform']]
}
    ...
    for rows in params["rows"]:
        for cols in params["cols"]:
            for kwargs in params["kwargs"]:
                ...
                parallel_transformer = ParallelQuantileTransformer(**kwargs, n_jobs=8)
