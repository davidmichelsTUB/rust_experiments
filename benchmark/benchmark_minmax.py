...
from benchmark_adapter import RustyMinMaxScaler
...
def _time_fit_transform(scaler, X: np.ndarray) -> tuple[float, float]:
    ...
    if isinstance(scaler, RustyMinMaxScaler):
        scaler.fit(X, 24)
    else:
        scaler.fit(X)
...
def main():
    warmup()
    with open("benchmark/results/minmax_benchmark.csv", "w+") as f:
    ...
    for n_cols, n_rows in product(n_cols_list, n_rows_list):
        ...
        with open("benchmark/results/minmax_benchmark.csv", "a") as f:
            f.write(f"\n{n_rows},{n_cols},")
