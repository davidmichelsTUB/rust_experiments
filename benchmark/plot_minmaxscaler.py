...
df = pd.read_csv("benchmark/results/minmax_benchmark.csv")
UNIT = "ms"
...
for i, nr in enumerate(n_rows_vals):
    ...
    ax.bar(x - width / 2, sub["baseline_fit"], width, color=colors[("sklearn", "fit")])
    ax.bar(
        x - width / 2,
        sub["baseline_transform"],
        width,
        bottom=sub["baseline_fit"],
        color=colors[("sklearn", "transform")],
    )

    # rust: same stacking
    ax.bar(x + width / 2, sub["rust_fit"], width, color=colors[("rust", "fit")])
    ax.bar(
        x + width / 2,
        sub["rust_transform"],
        width,
        bottom=sub["rust_fit"],
        color=colors[("rust", "transform")],
    )
...
fig.legend(
    handles=legend_handles, ncol=4, loc="upper center", bbox_to_anchor=(0.5, 1.0)
)
fig.suptitle(
    "sklearn vs Rust MinMaxScaler  (fit + transform stacked)", fontsize=14, y=1.06
)
fig.tight_layout()
fig.savefig(
    f"benchmark/results/benchmark_{sys.argv[1]}.png", dpi=150, bbox_inches="tight"
)
plt.show()
